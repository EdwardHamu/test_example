...
- 对比来源：`Arena模型探测工具-v2026.9.16.6-x64-extracted/{app}/assets/arena-model-probe.inject.js`。
- 逐模块比对后，未发现需要补迁移的独有功能。`native-capture`、`registry`、`classify`、`idmap`、`learned`、`probe` 六个模块与目标完全一致。
- 旧版显式思考强度、预算采集、Trace 强度字段解析和展示入口已存在于目标，相关实现已由此前的新版本迁移增强。目标额外保留完成帧识别、通知和自动 Esc，并包含 Trace 详情与内部档位信息。
- 没有用旧实现替换现有的范围校验、读取上限、敏感字段过滤或新版汇总逻辑，避免回退。目标脚本本次没有修改；仅补充此核对记录。
