(()=>{
 ...
 // The current conversation's "More options" button in the sidebar. The row markup moves between
 // site releases, so search the link's own container first and then up to three ancestor levels.
 const renameMenuButton=()=>{
  ...
  for(let node=links[0].parentElement,depth=0;node&&depth<4;node=node.parentElement,depth++){
   ...
   if(options.length===1)return {button:options[0]};
  }
  return {error:'未找到当前对话菜单'};
 };
 window.__arenaCandidate=(action,value)=>{try{
  ...
  if(action==='button'){
   ...
   const b=value==='Rename'?all('[role=menuitem]').filter(e=>/^(Rename|重命名)$/.test(label(e))):button(value);if(b.length!==1)throw Error('候选按钮不唯一或尚未出现：'+value);b[0].click();return {ok:true};
  }
  if(action==='file'){
  ...
  if(action==='renameFill'||action==='renameSave'||action==='renameCancel'){
   const ds=all('[role=dialog]').filter(e=>/^(Rename chat\b|重命名(?:聊天|对话)?)/.test(e.innerText));
   ...
   const b=all('button',d).filter(e=>(action==='renameSave'?/^(Rename|重命名|保存)$/:/^(Cancel|取消)$/).test(label(e)));
  ...
  if(action==='state')return {
   ...
   renameDialog:all('[role=dialog]').some(e=>/^(Rename chat\b|重命名(?:聊天|对话)?)/.test(e.innerText)),
   renameMenu:all('[role=menuitem]').some(e=>/^(Rename|重命名)$/.test(label(e))),
   renameSaveReady:all('[role=dialog]').filter(e=>/^(Rename chat\b|重命名(?:聊天|对话)?)/.test(e.innerText)).some(d=>all('button',d).some(e=>/^(Rename|重命名|保存)$/.test(label(e))&&!e.disabled)),
  ...
  if(action==='ensureSidebar'){
   const b=all('button').filter(e=>/^(Expand sidebar|展开侧栏|展开侧边栏)$/.test(label(e)));if(b.length===1)b[0].click();return {ok:true};
  }
  if(action==='renameMenu'||action==='renameMenuClick'){
   const target=renameMenuButton();
   if(target.error)throw Error(target.error);
   if(action==='renameMenuClick')target.button.click();
   else target.button.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,button:0,pointerType:'mouse',ctrlKey:false}));
   return {ok:true};
  }
  throw Error('未知候选操作');
