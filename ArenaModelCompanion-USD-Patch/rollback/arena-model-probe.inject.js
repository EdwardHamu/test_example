...
function boot(opts = {}) {
  ...
  const api = {
    ...
    acceptTraceDetail: (context, detail) => {
      ...
      desktopDetail={url:context.url,runId:context.runId,generation:context.generation,summary};BUS.mark('detail-accepted');return true;
    },
...
/* Arena right-side monitor v1.2.0. Read-only consumer of the existing probe API. */
(() => {
  'use strict';
  if (location.origin !== 'https://arena.ai' || window.top !== window) return;
  const KEY = '__ARENA_RIGHT_MONITOR_V1__';
  if (window[KEY]) { window[KEY].refresh(); return; }
  const CACHE='arena-monitor-last-confirmed-v1';
  let remembered=[];
  try {const v=JSON.parse(localStorage.getItem(CACHE)||'[]');if(Array.isArray(v))remembered=v.filter(r=>r&&typeof r.path==='string'&&/^https:\/\/arena\.ai\/agent\/[0-9a-f-]{36}\/?$/i.test(r.path)&&typeof r.model==='string'&&r.model.length<=200&&Number.isFinite(r.at)).slice(0,12);}catch(_){}
  const STORE = 'arena-monitor-preferences-v1';
  let prefs = {target:'',collapsed:false};
  try { const p=JSON.parse(localStorage.getItem(STORE)||'{}'); prefs.target=typeof p.target==='string'?p.target.slice(0,200):'';prefs.collapsed=p.collapsed===true; } catch (_) {}
  const save=()=>{try {localStorage.setItem(STORE,JSON.stringify(prefs));}catch (_) {}};
  const host=document.createElement('div');host.id='arena-right-model-monitor';
  host.style.cssText='all:initial!important;position:fixed!important;right:0!important;top:0!important;height:100dvh!important;z-index:2147483000!important;display:block!important;';
  const root=host.attachShadow({mode:'open'});
  root.innerHTML=`<style>
  :host{color-scheme:dark}*{box-sizing:border-box}aside{width:304px;height:100dvh;background:#202421;color:#edf2eb;border-left:1px solid #40493f;font:13px/1.6 system-ui,"Microsoft YaHei",sans-serif;display:flex;flex-direction:column;box-shadow:-6px 0 24px #0002}header{display:flex;align-items:center;justify-content:space-between;padding:18px 16px;border-bottom:1px solid #394239}h2{font-size:15px;margin:0}small{color:#9eafa1;font-size:11px}button{font:inherit;cursor:pointer;border:1px solid #4b5a4c;background:#30392f;color:#edf2eb;border-radius:7px;padding:7px 10px}button:hover{background:#42513f}button:focus-visible{outline:2px solid #b3d39d;outline-offset:2px}button:disabled{opacity:.4;cursor:not-allowed}.content{padding:16px;overflow:auto;flex:1}.label{font-size:11px;color:#a8b6a8;margin-bottom:6px;letter-spacing:.06em}.card{padding:13px;border:1px solid #3d483c;background:#272e26;border-radius:10px;margin-bottom:14px}.model{font-size:17px;font-weight:650;word-break:break-word;margin:7px 0}.status{display:inline-block;border-radius:20px;padding:3px 9px;background:#384038;color:#cad4c8;font-size:12px}.status.good{background:#25462f;color:#b4e6bb}.status.warn{background:#503d25;color:#f4cf8e}.buttons{display:flex;gap:7px;flex-wrap:wrap;margin-top:12px}.primary{background:#647853;border-color:#81976d}.target{word-break:break-word;color:#d4dfcc}.row{display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-bottom:1px solid #343d33}.row>span{color:#a5b3a3;flex-shrink:0}.row>b{font-weight:500;text-align:right;overflow-wrap:anywhere}p{margin:8px 0;overflow-wrap:anywhere}.note{font-size:11px;color:#9eafa1}.history{padding:8px 0;border-bottom:1px solid #364034;overflow-wrap:anywhere}.history small{display:block}.tab{display:none;margin-top:100px;border-radius:8px 0 0 8px;padding:14px 9px;writing-mode:vertical-rl;letter-spacing:3px;background:#2d3c2c}.closed aside{display:none}.closed .tab{display:block}footer{padding:12px 16px;border-top:1px solid #394239;color:#a5b3a3;font-size:11px}.warning{color:#f4cf8e} @media(max-width:700px){aside{width:min(304px,88vw)}}
  </style><div id="shell"><button class="tab" id="expand" aria-label="展开模型监测">模型监测</button><aside aria-label="模型监测面板"><header><div><h2>模型监测</h2><small>v1.5 · 多调用识别 · 已采集用量</small></div><button id="collapse" aria-label="收起模型监测">收起</button></header><div class="content">
  <div class="card"><div class="label">目标模型 · 完整名称精确匹配</div><div class="target" id="target"></div><div class="buttons"><button class="primary" id="set" disabled>将当前模型设为目标</button><button id="clear">清除</button></div></div>
  <div class="card"><span id="status" class="status" role="status" aria-live="polite">等待探针</span><div class="model" id="model">尚未识别</div><p id="reason" class="note"></p><p id="change" class="warning" role="status"></p></div>
  <div class="card"><div class="label">上次确认 · 仅历史，不代表本轮</div><div id="previous" class="target">本会话尚无确认记录</div><small id="previousAt"></small><div class="buttons"><button id="forget">清除此会话历史</button></div></div>
  <div class="row"><span>采集状态</span><b id="mainstate">等待证据</b></div><div class="row"><span>证据更新</span><b id="mainupdated">—</b></div>
  <details style="margin-top:14px"><summary>用量、型号与技术详情</summary><p id="usageScope" class="label">等待本轮用量</p><p id="usageNote" class="note"></p><div class="row"><span>内部档位</span><b id="tier">—</b></div><div class="row"><span>显式推理强度</span><b id="effort">未知</b></div><div class="row"><span>已采集输入 Token</span><b id="input">—</b></div><div class="row"><span>已采集输出 Token</span><b id="output">—</b></div><div class="row"><span>Trace 轮次</span><b id="turn">—</b></div><div class="row"><span>证据更新时间</span><b id="updated">—</b></div>
  <div class="row"><span>页面活动</span><b id="activity">—</b></div><div class="row"><span>采集连接</span><b id="connection">—</b></div><div class="row"><span>采集状态</span><b id="collection">—</b></div><div class="row"><span>生成标识</span><b id="generation">—</b></div><div class="row"><span>请求型号</span><b id="request">—</b></div><div class="row"><span>响应型号</span><b id="response">—</b></div><div class="row"><span>内部完整名称</span><b id="internal">—</b></div><div class="row"><span>平台路由</span><b id="provider">—</b></div><div class="row"><span>协议适配器</span><b id="protocol">—</b></div><div class="row"><span>推理 Token</span><b id="reasoningTokens">—</b></div><div class="row"><span>缓存输入 Token</span><b id="cached">—</b></div><div class="row"><span>距证据更新</span><b id="age">—</b></div><div class="row"><span>面板读取时间</span><b id="heartbeat">—</b></div><details style="margin-top:16px"><summary>本轮调用与证据详情</summary><p class="note" id="source"></p><div class="label">Run ID</div><p class="note" id="run"></p><pre id="calls" style="white-space:pre-wrap;overflow-wrap:anywhere;font:12px/1.7 inherit"></pre><p class="note" id="debug"></p></details>
</details>
  <p class="note">费用：此面板暂未接入桌面端费用账本，请以上方原有费用栏为准。内部档位不等于显式推理强度。</p>
  <details><summary>最近识别记录</summary><div class="label" style="margin-top:12px">最近识别记录 · 当前页面生命周期</div><div id="history" class="note">暂无记录</div></details>
  <details><summary>采集诊断（排障用）</summary><pre id="diagnosis" style="white-space:pre-wrap;overflow-wrap:anywhere;font-size:11px"></pre><button id="copydiag">复制诊断摘要</button><p id="copyresult" class="note"></p></details>
  </div><footer>不发送消息，不额外抽卡。模型标签仅代表平台暴露的信息；没有新证据时不会刷新确认时间。</footer></aside></div>`;
  const $=id=>root.getElementById(id),text=(id,value)=>{$(id).textContent=value==null?'—':String(value);};
  const layout=document.createElement('style');layout.id='arena-monitor-layout';
  let selected=null,lastConfirmed=null,lastScope='',lastApi=null,lastSignature='',changeNotice='',records=[];
  function arrange(){
    $('shell').classList.toggle('closed',prefs.collapsed);
    layout.textContent=prefs.collapsed?'':'@media(min-width:1000px){html{box-sizing:border-box!important;padding-right:304px!important;}}';
    text('target',prefs.target||'未设置 · 可将当前确认的模型设为目标');
  }
  $('collapse').onclick=()=>{prefs.collapsed=true;save();arrange();};
  $('expand').onclick=()=>{prefs.collapsed=false;save();arrange();};
  $('set').onclick=()=>{refresh();if(!selected)return;prefs.target=selected;save();arrange();refresh();};
  $('clear').onclick=()=>{prefs.target='';save();arrange();refresh();};
  function status(label,kind=''){text('status',label);$('status').className='status '+kind;}
  function empty(label,reason){selected=null;$('set').disabled=true;status(label);text('model','尚未确认');text('reason',reason);text('change','');for(const id of ['tier','input','output','turn','updated'])text(id,'—');text('effort','未知');}
  const number=n=>typeof n==='number'&&Number.isFinite(n)&&n>=0?n.toLocaleString():'—';
  const time=v=>{const d=new Date(v);return v&&Number.isFinite(d.getTime())?d.toLocaleTimeString():'—';};
  function previous(path){
    const p=remembered.find(r=>r.path===path);
    text('previous',p?p.model:'本会话尚无确认记录');text('previousAt',p?'确认于 '+new Date(p.at).toLocaleString()+' · 非当前轮确认':'目标名称不会自动成为识别结果');
  }
  $('forget').onclick=()=>{const path=location.origin+location.pathname;remembered=remembered.filter(r=>r.path!==path);try{localStorage.setItem(CACHE,JSON.stringify(remembered));}catch(_){}previous(path);};
  $('copydiag').onclick=async()=>{try{await navigator.clipboard.writeText($('diagnosis').textContent);text('copyresult','已复制，不含令牌、请求正文或聊天内容。');}catch(_){text('copyresult','剪贴板不可用，请截取下方诊断文字。');}};
  // Identity and usage are separate. Multi-call is not itself a model conflict.
  function summarizeMonitorCalls(input,limited=false){
    const unique=new Map();let duplicateConflict=false;
    for(const call of Array.isArray(input)?input:[]){
      if(!call||typeof call.spanId!=='string')continue;
      const prev=unique.get(call.spanId);
      if(prev){if(prev.requestModel!==call.requestModel||prev.responseModel!==call.responseModel)duplicateConflict=true;continue;}
      unique.set(call.spanId,call);
    }
    const calls=[...unique.values()],names=[...new Set(calls.map(c=>c.requestModel).filter(Boolean))];
    const conflict=duplicateConflict||calls.some(c=>c.requestModel&&c.responseModel&&c.requestModel!==c.responseModel);
    const incomplete=calls.some(c=>c.partial!==false||!c.requestModel);
    const state=!calls.length?'none':conflict?'conflict':names.length>1?'mixed':incomplete?'partial':'consistent';
    const sum=field=>{
      if(!calls.length||calls.some(c=>c.partial!==false))return null;
      let total=0;
      for(const c of calls){const n=c.usage?.[field];if(!Number.isSafeInteger(n)||n<0)return null;total+=n;if(!Number.isSafeInteger(total))return null;}
      return total;
    };
    return {calls,names,state,limited:!!limited,model:names.length===1?names[0]:null,complete:calls.filter(c=>c.partial===false).length,
      usage:{input:sum('input'),output:sum('output'),reasoning:sum('reasoning'),cachedInput:sum('cachedInput')},
      allUsageKnown:calls.length>0&&sum('input')!==null&&sum('output')!==null};
  }
  function refresh(){
    try {
      if(!document.documentElement)return;
      if(!host.isConnected)document.documentElement.append(host);
      if(!layout.isConnected)document.documentElement.append(layout);
      const path=location.origin+location.pathname,api=window.__MODEL_PROBE__;
      text('heartbeat',new Date().toLocaleTimeString());previous(path);
      if(!/^\/agent\/[0-9a-f-]{36}\/?$/i.test(location.pathname)){empty('等待打开会话','打开 Arena Agent 对话后开始监测。');clearDetails();lastScope='';lastConfirmed=null;return;}
      if(!api?.runState||!api?.desktopFacts){empty('等待探针','探针接口尚未就绪；不会自动发送测试消息。');clearDetails();return;}
      const run=api.runState(),facts=api.desktopFacts(),generation=api.bus?.generation;
      const snapshot=api.monitorSnapshot?.();const diag=snapshot?.diagnostics||null;
      const bound=snapshot?.url===path&&snapshot?.generation===generation&&snapshot?.runId===run.runId?snapshot.detail:null;
      const scope=path+'|'+generation;
      if(lastApi!==api||lastScope!==scope){lastApi=api;lastScope=scope;lastSignature='';selected=null;}
      const native=api.nativeStatus?.()||{};
      const counts=diag?.counts||{};
      const events=diag?.events||[];
      const diagText=['面板 v1.5 · 仅诊断元数据','探针：'+(api.version||'未知'),'实例：'+(diag?.instance||'未提供'),'启动时间：'+(diag?.bootAt?new Date(diag.bootAt).toLocaleString():'未提供'),'生成标识：'+generation,'CDP连接：'+(native.connected?'是':'否'),'原生事件：'+JSON.stringify(counts),'令牌校验：'+(diag?.tokenStatus||'尚无记录'),'Run状态：'+(run.runId?'已取得':'未取得'),'页面绑定：'+(run.tokenUrl===path?'匹配':'不匹配／未绑定'),'已校验明细：'+(snapshot?.detail?'有':'无'),'Trace已读取次数：'+(run.fetchCount||0),'最近状态重置：'+(diag?.lastReset||'未记录'),'最近事件：',...events.map(e=>time(e.at)+' '+e.kind+' '+(e.note||''))].join('\n');
      text('diagnosis',diagText);
      let page=null;try{page=window.__arenaCompanion?.read('');}catch(_){}
      text('activity',page?.generating?'回答／工具流程进行中':page?.responseComplete?'本次回答已完成':native.streaming>0?'数据流传输中':'等待新活动／状态未提供');
      text('connection',native.connected?'桌面 CDP 已连接':'页面采集模式／CDP 未连接');
      text('generation',generation);text('run',run.runId||'尚未取得');
      const states={pending:'等待证据',collected:'已采集',unprovided:'已采集，档位未提供',multi:'多调用，不能唯一归属',conflict:'证据冲突',incomplete:'采集不完整', 'rate-limited':'采集被限流'};
      text('collection',states[facts.collectionStatus]||facts.collectionStatus||'等待证据');text('mainstate',states[facts.collectionStatus]||'等待证据');
      text('debug','页面匹配：'+(run.tokenUrl===path?'是':'否')+' · 已校验明细：'+(bound?'有':'无')+' · 最近轮次：'+(run.lastSeenTurn??'—')+' · 基线：'+(run.minTurn??'—'));
      const history=run.modelHistory||[],entry=history[history.length-1];
      const traceCurrent=run.tokenUrl===path&&!!run.runId&&run.lastSeenTurn>run.minTurn;
      const detailCurrent=!!bound&&Number.isSafeInteger(bound.turn)&&bound.turn>0;
      const current=traceCurrent||detailCurrent;
      if(!current){
        let reason='等待绑定当前页面和生成标识的证据。';
        if(!run.runId)reason=diag?.tokenStatus&&diag.tokenStatus!=='accepted'&&diag.tokenStatus!=='duplicate'?'已观察到运行令牌但未通过校验，详见采集诊断。':native.connected?'CDP连接正常，但本探针尚未取得运行标识。':'采集尚未取得运行标识；连接状态不代表识别成功。';
        else if(run.tokenUrl!==path)reason='已有运行标识与当前页面未绑定，旧结果不用于本轮。';
        else reason='已取得运行标识，等待本轮 Trace；未放宽旧轮次校验。';
        empty('等待本轮识别',reason);clearEvidence();text('mainupdated','本轮尚无证据');text('source','等待当前轮证据');return;
      }
      const detail=detailCurrent?bound:null,turn=detail?.turn||run.lastSeenTurn;
      const assessment=detail?summarizeMonitorCalls(detail.calls,detail.collectionLimited):null;
      const calls=assessment?.calls||[];
      const names=detail?assessment.names:entry?.runId===run.runId?(entry.all||[entry.name]):[];
      const labels=[...new Set(names.filter(v=>typeof v==='string'&&v))];
      const mixed=detail?assessment.state==='mixed':labels.length>1;
      const conflict=detail?assessment.state==='conflict':false;
      const model=detail?assessment.model:run.modelName;
      const confirmed=detail?assessment.state==='consistent':!!model&&labels.length===1&&labels[0]===model&&entry?.runId===run.runId;
      selected=confirmed?model:null;$('set').disabled=!confirmed;
      const phase=conflict?'请求／响应标识冲突':mixed?'本轮已采集到不同型号':confirmed?'已采集调用型号一致':calls.length?'调用信息尚未完整':'等待调用明细';
      if(conflict)status('模型证据冲突','warn');
      else if(mixed)status('已采集调用含不同型号','warn');
      else if(!confirmed)status('本轮证据仍在采集中');
      else if(!prefs.target)status('已采集调用型号一致');
      else if(model===prefs.target)status('已采集调用均符合目标','good');
      else status('已采集调用不符合目标','warn');
      text('model',mixed?labels.join(' / '):model||'尚未确认');
      let note=detail?'已采集 '+calls.length+' 次调用，'+assessment.complete+' 次完整。':'来源：当前轮运行记录标签。';
      note+=conflict?'请求与响应型号不一致，不能确认命中。':mixed?'不同型号分别列在调用详情，不合并成单一模型。':confirmed?'仅描述已有证据；后续调用仍会继续检查。':'未完整或缺少型号的调用不算确认。';
      if(assessment?.limited)note+='采集有截断或停止标记，不代表本轮全部调用。';
      text('reason',note);
      text('collection',phase);text('mainstate',phase);
      if(run.collectionStatus==='rate-limited'){status('采集限流 · 保留已有证据','warn');text('reason','未额外重试；等待原探针采集恢复。');}
      else if(run.lastError){status('采集异常 · 保留已有证据','warn');text('reason','探针最近一次采集未成功，当前信息可能滞后。');}
      text('source',detail?'当前页面／生成标识／Run 已校验的 Trace 明细':'当前轮运行记录标签');
      text('change','');
      const at=detail?.checkedAt||facts.checkedAt||entry?.at;
      if(confirmed){
        if(lastConfirmed&&lastConfirmed.path===path&&lastConfirmed.model!==model)changeNotice='模型变化：'+lastConfirmed.model+' → '+model;
        else if(!lastConfirmed||lastConfirmed.path!==path)changeNotice='';
        text('change',changeNotice);
        const sig=scope+'|'+run.runId+'|'+turn+'|'+model+'|'+calls.map(c=>c.spanId).join(',')+'|'+at;
        if(sig!==lastSignature){
          lastSignature=sig;records=records.filter(r=>r.runId!==run.runId||r.turn!==turn||r.scope!==scope);records.unshift({model,turn,at,scope,runId:run.runId,count:calls.length});records=records.slice(0,12);
          const atMs=new Date(at).getTime();
          if(at&&Number.isFinite(atMs)){remembered=[{path,model,at:atMs},...remembered.filter(r=>r.path!==path)].slice(0,12);try{localStorage.setItem(CACHE,JSON.stringify(remembered));}catch(_){}previous(path);}
          const list=$('history');list.replaceChildren();
          for(const record of records){const row=document.createElement('div');row.className='history';row.textContent=record.model;const meta=document.createElement('small');meta.textContent='Trace 轮次 '+record.turn+' · '+(record.count||1)+' 次已采集调用 · '+time(record.at);row.append(meta);list.append(row);}
        }
        lastConfirmed={path,scope,model};
      }
      const d=detail||{};
      const shared=key=>{const values=[...new Set(calls.map(c=>c[key]).filter(Boolean))];return values.length>1?'多种：'+values.join(' / '):values.length===1?(calls.every(c=>!!c[key])?'':'部分提供：')+values[0]:'未提供';};
      text('request',mixed?labels.join(' / '):model||'未提供');text('response',detail?shared('responseModel'):'未提供');text('internal',d.internalModel||(calls.length>1?'多调用，未做跨调用归属':'未提供'));
      text('provider',calls.length>1?shared('modelProvider'):d.modelProvider||shared('modelProvider'));text('protocol',calls.length>1?shared('protocolProvider'):d.protocolProvider||shared('protocolProvider'));
      text('tier',d.internalTier||d.internalNameHint||(calls.length>1?'未按调用提供':'未提供'));text('effort',facts.effort?.display||'未知');
      const usage=calls.length===1&&detail?.usage?detail.usage:assessment?.usage||(!detail?run.usage:null);
      text('usageScope',detail?'已采集 '+calls.length+' 次调用合计（按 Span 去重）':'最新运行记录用量');
      text('input',number(usage?.input));text('output',number(usage?.output));text('reasoningTokens',number(usage?.reasoning));text('cached',number(usage?.cachedInput));
      text('usageNote',(assessment?.allUsageKnown?'不重复累加 usage/cost 事件。':'有缺失或未完成数据的指标不汇总；横线不代表 0。')+' 仅统计已采集调用，可能包含重试，不等于完整账单。');
      text('turn',turn);text('updated',time(at));text('mainupdated',time(at));
      const date=new Date(at).getTime();text('age',at&&Number.isFinite(date)?Math.max(0,Math.floor((Date.now()-date)/1000))+' 秒前':'未提供');
      const callText=calls.map((c,i)=>(i+1)+'. '+(c.requestModel||'型号未提供')+' · '+(c.partial?'采集中':'已完整')+'\n响应型号：'+(c.responseModel||'未提供')+'\n输入 '+number(c.usage?.input)+' / 输出 '+number(c.usage?.output)+' / 推理 '+number(c.usage?.reasoning)+'\nSpan：'+(c.spanId||'未提供')).join('\n\n');
      text('calls',callText||'暂未取得逐调用明细');
    }catch(_){empty('监测暂不可用','探针数据结构可能已变化；原有聊天与抽卡不受面板控制。');clearDetails();}
  }
  function clearEvidence(){text('usageScope','等待本轮用量');text('usageNote','缺失值不作为 0。');for(const id of ['request','response','internal','provider','protocol','reasoningTokens','cached','age'])text(id,'—');text('calls','等待本轮调用明细');}
  function clearDetails(){clearEvidence();text('mainstate','等待探针／会话');text('mainupdated','—');text('diagnosis','请打开会话，等待探针接口。');for(const id of ['activity','connection','generation','run','collection','source','debug'])text(id,'—');}
  arrange();window[KEY]={refresh};refresh();setInterval(refresh,1000);
})();
