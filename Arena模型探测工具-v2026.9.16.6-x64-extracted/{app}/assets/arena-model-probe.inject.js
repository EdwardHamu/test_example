/* arena-model-probe v1.0.0+cf897f9f — 单文件注入版 (CDP / DevTools Snippet) */
(function () {
"use strict";
var __mods = {}, __cache = {};
function __req(id) {
...
const SKIP = /^(?:messages?|parts|content|text|delta|prompt|input|output|choices|candidates|headers|token|authorization)$/i;
function extractReasoning(node, source = 'request', path = '$', depth = 0, out = []) {
  ...
  for (const [key, value] of Object.entries(node)) {
    ...
    const direct = /^(reasoning_effort|reasoningEffort|thinkingLevel|thinking_level)$/.test(key);
    const nested = key === 'effort' && /^(reasoning|output_config|outputConfig)$/.test(parent);
    if ((direct || nested) && typeof value === 'string') {
      const level = value.trim().toLowerCase();
      out.push({ kind: 'effort', level: EFFORT_LEVELS.includes(level) ? level : null,
        raw: value.slice(0, 60), status: EFFORT_LEVELS.includes(level) ? 'explicit' : 'unsupported', source, path: p });
    } else if (/^(thinkingBudget|thinking_budget|budget_tokens)$/.test(key)
      && /^(thinking|thinkingConfig|thinking_config)$/.test(parent) && Number.isFinite(value)) {
      out.push({ kind: 'budget', value, source, path: p });
    } else if (value && typeof value === 'object') extractReasoning(value, source, p, depth + 1, out);
  }
  return out;
}
function summarizeReasoning(evidence = []) {
  const items = evidence.filter(e => e && e.source === 'reasoning.config' && !e.stale).map(e => e.config);
  const efforts = items.filter(e => e && e.kind === 'effort');
  const levels = [...new Set(efforts.filter(e => e.level).map(e => e.level))];
  const unsupported = efforts.some(e => !e.level);
  return { status: levels.length > 1 ? 'conflict' : unsupported ? 'unsupported' : levels.length ? 'explicit' : 'unknown',
    level: levels.length === 1 && !unsupported ? levels[0] : null,
    evidence: items.slice(-12),
    note: levels.length > 1 ? '观察到不同强度，未强行合并；请按具体调用复核。'
      : levels.length ? '显式配置值，不代表实际计算量；不同厂商档位不可直接比较。'
      : '未观察到显式强度字段；不按耗时、Token 数或名称后缀猜测。' };
}
...
function parseTrace(text) {
  ...
  const visit = (obj, depth = 0) => {
    ...
    if (/^ai\.(?:streamText\.doStream|generateText\.doGenerate)$/.test(obj.message || obj.name || '')) {
      ...
      const reasoning = extractReasoning(obj.attributes || {}, 'run.trace');
      for (const [key, value] of Object.entries(obj.attributes || {})) {
        if (/^(?:ai\.settings\.reasoningEffort|gen_ai\.request\.reasoning_effort)$/.test(key)) reasoning.push(...extractReasoning({reasoning_effort:value}, 'run.trace', '$.attributes'));
      }
...
function acceptToken(name, value) {
  ...
  const scope = scopes.find(s => /^read:runs:run_[A-Za-z0-9]+$/.test(s));
...
async function fetchRunModels(opts = {}) {
  ...
  STATE.fetching = true;
  STATE.fetchCount++;
  const runId = STATE.runId, token = STATE.token, generation = BUS.generation;
...
async function pollRunModels(opts = {}) {
  ...
  while (Date.now() - t0 < maxMs) {
    ...
    if (['token-expired','no-token','superseded','http-401','http-403','http-404'].includes(last.reason)) return last;
  ...
  BUS.on(async (evt) => {
    ...
    if (r && r.ok && r.name) {
      ...
      for(let i=0;i<30;i++) {
        ...
        if(STATE.runId!==runId||BUS.generation!==generation)return;
        if(['token-expired','no-token','superseded','http-401','http-403','http-404','http-429'].includes(next.reason))return;
